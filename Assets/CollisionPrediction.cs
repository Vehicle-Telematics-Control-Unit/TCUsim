    ...
    void Update()
    {
        ...
        foreach (Vehicle v in surrounding_vehicles)
        {
            if ((my_vehicle.angle == 90 || my_vehicle.angle == 270) && (v.angle == 90 || v.angle == 270))
            {
                ...
                {
                    Debug.Log("Collision :: no collision");
                    continue;

                }
                else
                {
                    if (Math.Abs(my_vehicle.x - v.x) <= RADIUS * 2)
                    {
                    ...
                    {
                        Debug.Log("Collision :: Calculating time");
                        if (my_vehicle.angle == v.angle)
                        {
                            time2Collide = Math.Abs(my_vehicle.x - v.x - RADIUS * 2) / Math.Abs(my_vehicle.speed - v.speed);
                            Debug.Log("Collision :: Calculating time, Angles are equal");

                        }
                        else
                        {
                            time2Collide = Math.Abs(my_vehicle.x - v.x - RADIUS * 2) / (my_vehicle.speed + v.speed);
                            Debug.Log("Collision :: Calculating time, Angles aren't equal");
            ...
            // else
            // {
            //     if (Math.Tan(my_vehicle.angle) == Math.Tan(v.angle))
            //     {
            //         float m = (float)Math.Tan(my_vehicle.angle);
            ...
            //         && Math.Abs(my_vehicle.x - v.x) > RADIUS * 2 && Math.Abs(my_vehicle.z - v.z) > RADIUS * 2))
            //             continue;
            //         else
            //         {
